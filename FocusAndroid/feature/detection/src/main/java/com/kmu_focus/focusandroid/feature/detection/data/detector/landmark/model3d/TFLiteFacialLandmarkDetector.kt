package com.kmu_focus.focusandroid.feature.detection.data.detector.landmark.model3d
...
import com.kmu_focus.focusandroid.feature.detection.domain.entity.Vertex2D
import com.kmu_focus.focusandroid.feature.detection.domain.entity.Vertex3D
...
class TFLiteFacialLandmarkDetector @Inject constructor(
    ...
    override fun detectLandmarks(frame: Bitmap, faceRect: Rect): Face3DMMResult? {
    ...
    private fun parseLandmarks(output: FloatArray): Pair<List<Vertex2D>, List<Vertex3D>?> {
    ...
    private fun parseLandmarksQuantized(output: ByteArray): Pair<List<Vertex2D>, List<Vertex3D>?> {
        val list = mutableListOf<Vertex2D>()
        val list3D = if (numChannels == 3) mutableListOf<Vertex3D>() else null
        val stride = numChannels
