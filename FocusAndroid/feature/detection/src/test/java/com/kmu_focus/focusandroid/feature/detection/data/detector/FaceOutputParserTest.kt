...
class FaceOutputParserTest {
    ...
    @Test
    fun `단일 얼굴을 올바르게 파싱함`() {
        // YuNet 15-col: [x, y, w, h, re_x, re_y, le_x, le_y, nose_x, nose_y, rm_x, rm_y, lm_x, lm_y, confidence]
    ...
    fun `스케일 팩터가 좌표에 올바르게 적용됨`() {
        ...
        val face = result[0]
        assertEquals(20, face.x)     // 10 * 2
        assertEquals(60, face.y)     // 20 * 3
        assertEquals(100, face.width) // 50 * 2
        assertEquals(180, face.height) // 60 * 3
    }
